package com.init.utils;

import com.init.dto.BrandRequest;
import com.init.dto.ModelRequest;
import lombok.extern.slf4j.Slf4j;

import java.math.BigDecimal;
import java.util.regex.Pattern;

@Slf4j
public class Validation {

    public static String validateBrandRequest(BrandRequest brandRequest){
        if (!Validation.validLength(brandRequest.getName(),2,50))
            return "Brand name must be between 2 and 50 characters";
        return null;
    }
    public static String validateModelRequest(ModelRequest modelRequest){
        if (!Validation.validLength(modelRequest.getName(),2,50))
            return "Model name must be between 2 and 50 characters";
        if (modelRequest.getExterior()!=null && !Validation.validLength(modelRequest.getExterior(),2,16))
            return "Exterior color should be between 2 and 16 characters color code";
        if (!Validation.validLength(modelRequest.getInterior(),2,16))
            return "Interior color should be between 2 and 16 characters color code";
        if (!Validation.validLength(modelRequest.getYear(),4))
            return "Year must be 4 digits";
        BigDecimal price = new BigDecimal(modelRequest.getPrice());
        if (price.compareTo(BigDecimal.ZERO)< 0)
            return "Negative price value is not allowed";
        return null;
    }
    public static boolean validData(Object obj) {
        if (obj == null || String.valueOf(obj).isEmpty()) {
            return false;
        }
        if (obj.equals("null")) {
            return false;
        }
        if (obj instanceof String) {
            String s = ((String) obj).trim();
            if (s.length() < 1) return false;
        }
        if (obj.equals(0))
            return false;
        return true;
    }

    public static boolean validName(String name) {

        if (name.length() < 2) {
            return false;
        }

        if (name.length() > 255) {
            return false;
        }

        //pure alphabets
        if (!(Pattern.compile("[a-zA-Z]+").matcher(name).matches())) {
            return false;
        }

        return true;
    }

    public static boolean validLength(String string, int min, int max) {

        if (!validData(string)) return false;

        if (string.length() < min) {
            return false;
        }

        if (max != 0) {
            if (string.length() > max) {
                return false;
            }
        }

        return true;
    }

    public static boolean validNumberLength(String numbers, int min, int max) {

        //pure number
        if (!validNumber(numbers)) {
            return false;
        }
        if (!validLength(numbers, min, max))
            return false;

        return true;
    }

    public static boolean validNumberLength(String numbers, int digits) {
        if (numbers == null)
            return false;

        //pure number
        if (!validNumber(numbers)) {
            return false;
        }

        if (!validLength(numbers, digits))
            return false;

        return true;
    }

    public static boolean validNumber(String numbers) {
        //pure number
        if (!(Pattern.compile("[0-9]+").matcher(numbers).matches())) {
            return false;
        }
        return true;
    }

    public static boolean validLength(String string, int digits) {

        if (string.length() < digits) {
            return false;
        }

        if (digits != 0) {
            if (string.length() > digits) {
                return false;
            }
        }
        return true;
    }

    public static void main(String[] args) {
        String character = "";
        byte[] bytes = character.getBytes();
        System.out.println("The lenght of character " + bytes.length);
    }
}
